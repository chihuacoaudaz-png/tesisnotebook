# -*- coding: utf-8 -*-
"""
ENSAMBLADOR MAESTRO Y FINAL DEL PLAN DE TESIS OFICIAL UNI FIGMM (52 PÁGINAS CONTINUAS)
...
def build_pure_plan_de_tesis(pdf_filename="output/PLAN_DE_TESIS_OFICIAL_UNI_50PAGS.pdf"):
    ...
    style_cover_univ = ParagraphStyle('CoverUniv_Plan', fontName='Helvetica-Bold', fontSize=15, leading=19, alignment=1, textColor=c_primary)
    style_cover_fac = ParagraphStyle('CoverFac_Plan', fontName='Helvetica-Bold', fontSize=11, leading=14.5, alignment=1, textColor=c_secondary)
    style_cover_title = ParagraphStyle('CoverTitle_Plan', fontName='Helvetica-Bold', fontSize=11.5, leading=15.5, alignment=1, textColor=c_primary, spaceBefore=8, spaceAfter=10)
    style_cover_meta = ParagraphStyle('CoverMeta_Plan', fontName='Helvetica', fontSize=9, leading=13, alignment=1, textColor=c_dark)
    
    style_h1 = ParagraphStyle('Heading1_Plan', fontName='Helvetica-Bold', fontSize=11.5, leading=15, textColor=c_primary, spaceBefore=14, spaceAfter=6, keepWithNext=True)
    style_h2 = ParagraphStyle('Heading2_Plan', fontName='Helvetica-Bold', fontSize=10, leading=13, textColor=c_secondary, spaceBefore=10, spaceAfter=4.5, keepWithNext=True)
    style_h3 = ParagraphStyle('Heading3_Plan', fontName='Helvetica-Bold', fontSize=8.8, leading=12, textColor=c_dark, spaceBefore=8, spaceAfter=3.5, keepWithNext=True)
    style_body = ParagraphStyle('Body_Plan', fontName='Helvetica', fontSize=8.5, leading=12.2, alignment=4, textColor=c_dark, spaceAfter=5.5)
    style_bullet = ParagraphStyle('Bullet_Plan', fontName='Helvetica', fontSize=8.5, leading=12, alignment=4, leftIndent=12, textColor=c_dark, spaceAfter=3.5)
    style_eq = ParagraphStyle('Equation_Plan', fontName='Helvetica-Bold', fontSize=8.0, leading=11, alignment=1, textColor=c_primary, spaceBefore=4.5, spaceAfter=5.5)
    style_caption = ParagraphStyle('Caption_Plan', fontName='Helvetica-Oblique', fontSize=7.2, leading=9.5, alignment=1, textColor=c_muted, spaceBefore=3.0, spaceAfter=6.0)
    style_th = ParagraphStyle('TableHeader_Plan', fontName='Helvetica-Bold', fontSize=7.0, leading=9.0, alignment=1, textColor=colors.white)
    style_td = ParagraphStyle('TableCell_Plan', fontName='Helvetica', fontSize=6.8, leading=8.5, textColor=c_dark)
    style_code = ParagraphStyle('Code_Plan', fontName='Courier', fontSize=6.5, leading=8.2, textColor=c_primary)
    ...
    story.append(Paragraph("<b>PLAN DE TESIS</b>", ParagraphStyle('TW_Plan', fontName='Helvetica-Bold', fontSize=13.0, alignment=1, textColor=c_primary)))
    story.append(Spacer(1, 0.15 * cm))
    ...
    # =========================================================================
    # SECCIONES ESTRUCTURALES DEL PLAN DE TESIS (1:1 con PLAN DE TESIS (1).docx)
    # =========================================================================
    ...
    from generate_plan_part2 import append_plan_part2
    append_plan_part2(story, p, pb, peq, ph1, ph2, ph3, pcap, style_th, style_td, style_code, colors, c_primary, c_border, c_bg_light, cm)
    ...
    print(f"[EXITO] Plan de Tesis generado con éxito. Total de páginas físicas: {num_pages}")
    return num_pages

if __name__ == "__main__":
    build_pure_plan_de_tesis()
