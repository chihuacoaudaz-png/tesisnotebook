# -*- coding: utf-8 -*-
"""
COMPILADOR MAESTRO Y FINAL DE TESIS OFICIAL UNI FIGMM (52-54 PÁGINAS CONTINUAS Y DENSAS)
Asegura que el PDF tenga exactamente >= 52 páginas y <= 55 páginas sin saltos artificiales entre subtítulos.
"""
...
def compile_dense_thesis(expansion_level=1, pdf_filename="output/TESIS_OFICIAL_UNI_LINCUNA_50PAGS.pdf"):
    ...
    # Inyectar expansiones geomecánicas y analíticas según nivel
...
def run_adaptive_compilation():
    target_min = 52
    target_max = 55
    
    # 1. Primera compilación
    compile_dense_thesis(expansion_level=1)
    reader = pypdf.PdfReader("output/TESIS_OFICIAL_UNI_LINCUNA_50PAGS.pdf")
    num_pages = len(reader.pages)
    print(f"[*] Conteo actual de páginas: {num_pages}")
    
    if num_pages >= target_min and num_pages <= target_max:
        print(f"[EXITO] La tesis cumple exactamente con el requerimiento ({num_pages} páginas).")
        return num_pages
        
    return num_pages

if __name__ == "__main__":
    run_adaptive_compilation()
