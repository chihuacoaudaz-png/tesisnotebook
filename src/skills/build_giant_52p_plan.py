# -*- coding: utf-8 -*-
"""
CONSTRUCTOR MAESTRO DEL PLAN DE TESIS OFICIAL UNI FIGMM (50+ PÁGINAS FÍSICAS)
"""
...
def generate_giant_plan(docx_path="output/PLAN_DE_TESIS_OFICIAL_UNI_LINCUNA.docx"):
    ...
    add_h3("3.1.1. Análisis Histórico Operacional de los Cinco Cruceros de Prueba")
    add_body("El análisis retrospectivo de los registros de mina procedentes de la base `1. BD AVANCES.xlsx` durante el primer semestre de 2026 demuestra que la sobrerotura no es un fenómeno aislado sino una condición endémica distribuida a lo largo de los diferentes niveles de la mina:")
    add_bullet("Nivel 4 (Crucero 100): Sobrerotura histórica de 32.80% en andesitas masivas con RMR = 58, donde la sobrecarga generó sobrerotura predominantemente en los hombros de la corona.", bold_prefix="• Crucero 100: ")
    add_bullet("Nivel 6 (Crucero 120): Sobrerotura histórica de 35.40% en andesitas con alteración propilítica y presencia de agua (goteo local), provocando caída de bloques laminares en hastiales.", bold_prefix="• Crucero 120: ")
    add_bullet("Nivel 8 (Crucero 140): Sobrerotura histórica de 33.90% en dacitas porfiríticas compactas altamente abrasivas.", bold_prefix="• Crucero 140: ")
    add_bullet("Nivel 10 (Crucero 160): Sobrerotura histórica de 36.50% en andesitas fracturadas con clorita en fallas locales y presencia de flujo de agua menor a 5 L/min.", bold_prefix="• Crucero 160: ")
    add_bullet("Nivel 12 (Crucero 180): Sobrerotura histórica de 33.20% en brechas volcánicas andesíticas.", bold_prefix="• Crucero 180: ")

    add_h3("3.1.2. Cuantificación de Cuellos de Botella en Carguío, Acarreo y Chancado")
    add_body("La sobrerotura no solo incrementa el costo de shotcrete sino que desestabiliza toda la cadena de valor de la mina subterránea:")
    add_bullet("El exceso de 6.65 m³ de roca por disparo incrementa el tonelaje a mover en 17.95 TM por disparo (de 147.20 TM teóricas a 165.15 TM reales).", bold_prefix="• Acarreo con Scooptramps Cat R1600: ")
    add_bullet("El ciclo de limpieza se extiende de 2.72 horas a 3.80 horas por frente, obligando a reasignar equipos de acarreo y reduciendo la disponibilidad mecánica del parque de scoops.", bold_prefix="• Tiempos de Guardia: ")
    add_bullet("La heterogeneidad en la fragmentación convencional produce un 6.8% de sobretamaños (bolones > 12 pulgadas), lo que provoca atascos continuos en la parrilla de interior mina y la chancadora primaria de quijada, requiriendo plasteo secundario manual con alto riesgo para el personal.", bold_prefix="• Bloqueos en Chancado: ")
