# -*- coding: utf-8 -*-
"""
GENERADOR MAESTRO DEL MARCO TEÓRICO EN FORMATO NATIVO DOCX CON FÓRMULAS OMML NATIVAS Y CERTIFICACIÓN >= 20 PÁGINAS
...
# Configurar XSLT oficial de Microsoft Office para MML a OMML
XSLT_PATH = r'C:\Program Files\Microsoft Office\root\Office16\MML2OMML.XSL'
if not os.path.exists(XSLT_PATH):
    # Alternativa si Office está en x86
...
def build_complete_marco_teorico_docx(output_docx):
    doc = docx.Document()
    
    # Márgenes UNI FIGMM (3.0 cm izq, 2.5 cm der, 2.54 cm sup, 2.5 cm inf)
    ...
    def add_eq(latex_code):
        ...
        try:
            ...
            print(f"Error convirtiendo LaTeX a OMML ({latex_code}): {e}")
    ...
    add_title("CAPÍTULO II: MARCO TEÓRICO Y CONCEPTUAL")
    add_caption("Proyecto de Tesis de Posgrado / Titulación Profesional - UNI FIGMM")
    ...
    # ------------------ 2.2. BASES TEÓRICAS ------------------
    add_h1("2.2. Bases Teóricas y Físico-Matemáticas")
    add_body("Las bases teóricas desarrollan la formulación físico-matemática rigurosa de las disciplinas involucradas: la clasificación geomecánica de Barton, la física de propagación de ondas dinámicas, la fenomenología del estallido de rocas y el balance de absorción de energía en el sostenimiento compuesto.")
    ...
    add_bullet("Cociente de Fricción Interfacial (Jr / Ja): Representa el coeficiente de fricción residual efectivo tan(phi_r) entre las paredes de las discontinuidades según el modelo constitutivo de Barton-Bandis.")
    ...
    add_body("Donde lambda y mu son las constantes elásticas de Lamé, rho_r es la densidad de la roca y u es el vector de desplazamiento de partícula. De esta formulación se derivan las velocidades de propagación de las ondas de cuerpo:")
    ...
    add_body("Donde R es la distancia hipocentral al punto de disparo en metros, W es la masa máxima de explosivo detonada en un retardo de 8 ms en kilogramos, K es el coeficiente de transmisión del terreno y beta es el exponente de atenuación inelástica.")
    ...
    add_body("Donde q es la concentración lineal de carga explosiva (kg/m), Hc es la longitud de columna cargada, r0 es la distancia perpendicular al contorno del túnel y z0 es la posición axial relativa.")
    ...
    add_body("El diseño de soporte dinámico se fundamenta en el principio de conservación y disipación de energía formulado por Ortlepp y Stacey (1994) y Kaiser et al. (1996): la capacidad de absorción de trabajo mecánico del sistema compuesto de sostenimiento debe ser estrictamente mayor o igual a la demanda de energía cinética y potencial impartida por la roca en colapso.")

    add_eq(r"E_{\text{demanda}} = \frac{1}{2} \rho_r \cdot t_b \cdot v_e^2 + \rho_r \cdot t_b \cdot g \cdot d")

    add_body("Donde m es la masa de roca inestable por unidad de superficie (m = rho_r * tb * A, siendo tb el espesor del halo plastificado), g es la aceleración de la gravedad (9.81 m/s^2), d es el desplazamiento elasto-plástico del soporte y ve es la velocidad de expulsión inicial del bloque de roca.")
    ...
    add_body("Donde eta es el factor de amplificación dinámica en la cara libre (eta = 1.5 a 3.0 para rocas andesíticas competentes).")
    ...
    add_body("Mientras que los pernos convencionales rígidos (Fully Grouted Rebar o Split Set) rompen a desplazamientos menores a 30 mm disipando menos de 4 kJ, los pernos dinámicos modernos (D-Bolt o Cone Bolt) fluyen plásticamente disipando entre 35 y 50 kJ por perno con elongaciones superiores a 150 - 250 mm.")

    add_bullet("Aporte de Malla Metálica de Alta Resistencia (E_malla): Malla romboidal de alambre de 1,770 MPa que absorbe entre 15 y 25 kJ/m^2 mediante deflexión en membrana antes de romper.")
    add_bullet("Aporte de Concreto Lanzado con Fibra FRS (E_shotcrete): Tenacidad a flexotracción evaluada bajo norma ASTM C1550 (paneles redondos), absorbiendo entre 600 y 800 Joules por panel a 40 mm de deflexión central.")
    ...
    add_h2("2.2.5. Modelo de Optimización Acoplada: Índice Qsismico y Matriz de Selección")
    ...
    data = [
        ...
        ("Cat. II: Dinámico Moderado", "150 - 350 mm/s", "1.0 <= Q < 4.0", "5 - 15 kJ/m²", "Pernos D-Bolt o helicoidales dúctiles (L=2.4m, 1.0x1.0m) + Malla electrosoldada reforzada + Shotcrete FRS e=7.5cm (400 J)."),
        ("Cat. III: Dinámico Pesado", "350 - 600 mm/s", "0.1 <= Q < 1.0", "15 - 35 kJ/m²", "Pernos D-Bolt/Garford (L=2.5m, 0.8x0.8m) + Malla romboidal 1,770 MPa + Shotcrete FRS e=10cm (>=600 J ASTM C1550)."),
        ("Cat. IV: Dinámico Extremo", "> 600 mm/s", "Q < 0.1", "> 35 kJ/m² (>=50 kJ)", "Doble contención: Pernos D-Bolt + Cables dinámicos bulbados (L=5-6m) + Malla romboidal con cable lacing + FRS doble capa e=12-15cm.")
    ]
    ...
    add_h1("2.3. Marco Conceptual Extenso (Glosario Enciclopédico Especializado)")
    add_body("A continuación se desarrollan 35 términos técnico-científicos esenciales de la investigación, estructurados bajo el estándar normativo de dos párrafos continuos por concepto: el primero dedicado a su definición formal según autores canónicos, y el segundo a su contextualización operacional y causal en la mina subterránea.")

    conceptos = [
        ("1. Sistema Q de Barton (Tunneling Quality Index)",
         "El Sistema Q es una clasificación geomecánica empírico-analítica desarrollada en 1974 por Nick Barton, Robert Lien y John Lunde en el Norwegian Geotechnical Institute (NGI) para dimensionar sostenimientos subterráneos. Se expresa en una escala logarítmica de 0.001 a 1,000 mediante el producto de tres cocientes que representan el tamaño relativo de bloques rocosos, la resistencia al corte residual entre juntas y el estado de confinamiento tensional activo e hidrogeológico.",
        ...
        ("5. Alteración de Juntas (Ja)",
         ...
         "En yacimientos polimetálicos andinos, donde la alteración hidrotermal degrada las cajas de veta, un valor elevado de Ja reduce drásticamente el ángulo de fricción residual inter-bloques (tan phi_r = Jr / Ja). Durante eventos dinámicos o disparos de voladura masiva, las juntas con alto Ja experimentan deslizamientos dinámicos instantáneos (slip), provocando eyecciones súbitas de roca incluso a intensidades moderadas de vibración."),
        ...
        ("10. Frecuencia Dominante de Vibración (fd)",
         "La frecuencia dominante es aquella que concentra la mayor densidad espectral de energía en el registro sismográfico, identificada mediante la Transformada Rápida de Fourier (FFT) y medida en Hertz (Hz). Define, en combinación con la velocidad de propagación de onda de la roca (cp), la longitud de onda física de la perturbación dinámica (lambda = cp / fd).",
        ...
        ("17. Sostenimiento Dinámico",
         ...
         "Constituye la variable dependiente de optimización de la presente investigación. La integración analítica del Sistema Q y el monitoreo de vibraciones permite seleccionar la combinación óptima de pernos dinámicos, mallas de alta resistencia y shotcrete con fibra para garantizar un factor de seguridad FS >= 1.5 ante la demanda energética específica del frente, optimizando costos y asegurando la vida humana."),
        ...
        ("19. Perno Dinámico de Deformación Plástica (D-Bolt)",
         ...
         "En labores subterráneas sometidas a altos niveles de vibración (PPV > 350 mm/s), el D-Bolt reemplaza con superioridad técnica a los pernos helicoidales rígidos. Cada D-Bolt de 22 mm se elonga más de 150 mm disipando entre 35 y 45 kJ de energía cinética por barra, evitando la falla por cizallamiento en discontinuidades activas y manteniendo el confinamiento de la corona."),
        ...
        ("34. Momento Sísmico (M0) y Energía Sísmica Radiada (Es)",
         "El momento sísmico (M0 = G * A * D) es la medida física fundamental del tamaño de un evento sísmico cosísmico, donde G es el módulo de rigidez de corte, A es el área de la falla que deslizó y D es el desplazamiento medio cosísmico, expresado en Newton-metro (N*m). La energía sísmica radiada (Es) representa la fracción de energía elástica que se propaga como ondas sísmicas destructivas.",
         "En la ingeniería de sostenimiento dinámico, el momento sísmico cuantifica la magnitud real del evento geomecánico que la labor debe soportar. Correlacionar el momento sísmico de los eventos inducidos con las mediciones de PPV en los cruceros permite calibrar los umbrales de energía de diseño para las diferentes categorías de fortificación."),
    ...
    add_h1("2.4. Referencias Bibliográficas (Formato APA 7ma Edición)")
    add_body("Las referencias bibliográficas se presentan en estricto orden alfabético y con sangría francesa conforme a las normas de estilo APA 7ma edición:")
    ...
    print(f"Documento DOCX generado exitosamente en: {output_docx}")

def certify_and_render_audit(docx_path, pdf_path):
    ...
    try:
        ...
        print(f"\n=======================================================")
        print(f"CERTIFICACIÓN FÍSICA EN MICROSOFT WORD:")
        print(f"- Páginas Físicas Reales: {pages} páginas")
        print(f"- Total Palabras: {words} palabras")
        print(f"- Total Caracteres: {chars}")
        print(f"=======================================================\n")

        print(f"Exportando PDF oficial en: {abs_pdf}")
        doc.SaveAs2(abs_pdf, FileFormat=17) # wdFormatPDF
        doc.Close()
        word.Quit()

        # Renderizar páginas de fórmulas para auditoría visual
        print("Renderizando páginas de fórmulas a imágenes PNG para auditoría visual...")
        doc_pdf = fitz.open(abs_pdf)
        # Vamos a renderizar páginas 4, 5, 6, 7 (donde están las fórmulas principales)
        os.makedirs("output/audit_pages", exist_ok=True)
        rendered_images = []
        for p_no in range(min(len(doc_pdf), 8)):
            page = doc_pdf[p_no]
            pix = page.get_pixmap(dpi=150)
            img_path = f"output/audit_pages/page_{p_no+1}.png"
            pix.save(img_path)
            rendered_images.append(img_path)
        print(f"Páginas renderizadas para auditoría: {rendered_images}")
        return pages, words, rendered_images
...
if __name__ == "__main__":
    final_docx = "output/CAP_II_MARCO_TEORICO_Q_VIBRACIONES_SOSTENIMIENTO_DINAMICO.docx"
    final_pdf = "output/CAP_II_MARCO_TEORICO_Q_VIBRACIONES_SOSTENIMIENTO_DINAMICO.pdf"
    
    os.makedirs("output", exist_ok=True)
    build_complete_marco_teorico_docx(final_docx)
    pages, words, imgs = certify_and_render_audit(final_docx, final_pdf)
    print("Auditoría lista.")
