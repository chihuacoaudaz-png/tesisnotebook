# -*- coding: utf-8 -*-
"""
COMPILADOR MAESTRO Y FINAL DEL PLAN DE TESIS OFICIAL UNI FIGMM (52 PÁGINAS CONTINUAS)
Estructura idéntica y 1:1 a PLAN DE TESIS (1).docx de pregrado UNI FIGMM.
"""
...
def build_official_plan_52p_pdf(pdf_filename="output/PLAN_DE_TESIS_OFICIAL_UNI_50PAGS.pdf"):
    ...
    style_cover_univ = ParagraphStyle('CoverUniv_P52', fontName='Helvetica-Bold', fontSize=15, leading=19, alignment=1, textColor=c_primary)
    style_cover_fac = ParagraphStyle('CoverFac_P52', fontName='Helvetica-Bold', fontSize=11, leading=14.5, alignment=1, textColor=c_secondary)
    style_cover_title = ParagraphStyle('CoverTitle_P52', fontName='Helvetica-Bold', fontSize=11.5, leading=15.5, alignment=1, textColor=c_primary, spaceBefore=8, spaceAfter=10)
    style_cover_meta = ParagraphStyle('CoverMeta_P52', fontName='Helvetica', fontSize=9, leading=13, alignment=1, textColor=c_dark)
    
    style_h1 = ParagraphStyle('Heading1_P52', fontName='Helvetica-Bold', fontSize=11.5, leading=15, textColor=c_primary, spaceBefore=14, spaceAfter=6, keepWithNext=True)
    style_h2 = ParagraphStyle('Heading2_P52', fontName='Helvetica-Bold', fontSize=10, leading=13, textColor=c_secondary, spaceBefore=10, spaceAfter=4.5, keepWithNext=True)
    style_h3 = ParagraphStyle('Heading3_P52', fontName='Helvetica-Bold', fontSize=8.8, leading=12, textColor=c_dark, spaceBefore=8, spaceAfter=3.5, keepWithNext=True)
    style_body = ParagraphStyle('Body_P52', fontName='Helvetica', fontSize=8.5, leading=12.2, alignment=4, textColor=c_dark, spaceAfter=5.5)
    style_bullet = ParagraphStyle('Bullet_P52', fontName='Helvetica', fontSize=8.5, leading=12, alignment=4, leftIndent=12, textColor=c_dark, spaceAfter=3.5)
    style_eq = ParagraphStyle('Equation_P52', fontName='Helvetica-Bold', fontSize=8.0, leading=11, alignment=1, textColor=c_primary, spaceBefore=4.5, spaceAfter=5.5)
    style_caption = ParagraphStyle('Caption_P52', fontName='Helvetica-Oblique', fontSize=7.2, leading=9.5, alignment=1, textColor=c_muted, spaceBefore=3.0, spaceAfter=6.0)
    style_th = ParagraphStyle('TableHeader_P52', fontName='Helvetica-Bold', fontSize=7.0, leading=9.0, alignment=1, textColor=colors.white)
    style_td = ParagraphStyle('TableCell_P52', fontName='Helvetica', fontSize=6.8, leading=8.5, textColor=c_dark)
    style_code = ParagraphStyle('Code_P52', fontName='Courier', fontSize=6.5, leading=8.2, textColor=c_primary)
    ...
    # =========================================================================
    # PORTADA OFICIAL SEGÚN PLANTILLA DE PREGRADO UNI FIGMM
    # =========================================================================
    ...
    story.append(Paragraph("<b>PLAN DE TESIS</b>", ParagraphStyle('TW_P52', fontName='Helvetica-Bold', fontSize=13.0, alignment=1, textColor=c_primary)))
    story.append(Spacer(1, 0.15 * cm))
    ...
    # =========================================================================
    # ENSAMBLAJE DE SECCIONES 1:1 CON EL PLAN DE PREGRADO
    # =========================================================================
    ...
    from generate_massive_plan_content_52p import append_massive_plan_content
    append_massive_plan_content(story, p, pb, peq, ph1, ph2, ph3, pcap, style_th, style_td, style_code, colors, c_primary, c_border, c_bg_light, cm)
    ...
    from expand_plan_to_52p import append_expanded_plan_sections
    append_expanded_plan_sections(story, p, pb, peq, ph1, ph2, ph3, pcap, style_th, style_td, style_code, colors, c_primary, c_border, c_bg_light, cm)
    ...
    from generate_massive_annexes_and_theory_52p import append_massive_annexes_and_theory
    append_massive_annexes_and_theory(story, p, pb, peq, ph1, ph2, ph3, pcap, style_th, style_td, style_code, colors, c_primary, c_border, c_bg_light, cm)

    from generate_deep_methodology_and_geomech_sheets import append_deep_methodology_and_sheets
    append_deep_methodology_and_sheets(story, p, pb, peq, ph1, ph2, ph3, pcap, style_th, style_td, style_code, colors, c_primary, c_border, c_bg_light, cm)

    doc.build(story, canvasmaker=NumberedPlanCanvas)
    
    reader = pypdf.PdfReader(pdf_filename)
    num_pages = len(reader.pages)
    print(f"[EXITO] Plan de Tesis Oficial compilado con éxito. Total de páginas: {num_pages}")
    return num_pages

if __name__ == "__main__":
    build_official_plan_52p_pdf()
