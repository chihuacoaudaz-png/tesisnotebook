...
def generate_pdf():
    pdf_filename = "output/TESIS_OFICIAL_UNI_LINCUNA_50PAGS.pdf"
    ...
    # 2. Dedicatoria y Agradecimientos (Página 2)
    ...
    # 4. Índices (Páginas 4 y 5)
    toc_p1 = [
        ("PORTADA OFICIAL", "i"),
        ("DEDICATORIA Y AGRADECIMIENTOS", "ii"),
        ("RESUMEN Y ABSTRACT", "iii"),
        ("ÍNDICE GENERAL", "iv"),
        ("ÍNDICE DE TABLAS E ÍNDICE DE FIGURAS", "vi"),
        ("INTRODUCCIÓN", "1"),
        ("CAPÍTULO I: PLANTEAMIENTO DEL ESTUDIO", "3"),
        ("   1.1. Contexto Operacional, Geográfico y Geológico en U.E.A. Lincuna", "3"),
        ("   1.2. Planteamiento de la Realidad Problemática de Sobrerotura", "5"),
        ("   1.3. Mecanismos Físicos y Geomecánicos de Sobre-excavación", "7"),
        ("   1.4. Modelo de Costos Unitarios y Sobrecostos en el Ciclo Minero", "9"),
        ("   1.5. Árbol de Causas Raíz, Problema Central y Efectos Críticos", "11"),
        ("   1.6. Cuadro Comparativo Situación Actual vs. Situación Futura", "13"),
        ("   1.7. Formulación del Problema (General y Específicos)", "15"),
        ("   1.8. Justificación Cuádruple de la Investigación", "17"),
        ("   1.9. Delimitación y Alcances del Estudio", "19"),
        ("   1.10. Objetivos de la Investigación (General y Específicos)", "20"),
        ("   1.11. Hipótesis de la Investigación (General y Específicas)", "21"),
    ]
    t_toc1_data = []
    for item, page_num in toc_p1:
        is_b = not item.startswith("   ")
        fn = 'Helvetica-Bold' if is_b else 'Helvetica'
        sz = 7.5 if not is_b else 8
        clr = c_primary if is_b else c_dark
        p_item = Paragraph(f"<b>{item}</b>" if is_b else item, ParagraphStyle('TOC1', fontName=fn, fontSize=sz, leading=sz+2, textColor=clr))
        p_num = Paragraph(f"<b>{page_num}</b>" if is_b else page_num, ParagraphStyle('TOCN1', fontName=fn, fontSize=sz, leading=sz+2, alignment=2, textColor=clr))
        t_toc1_data.append([p_item, p_num])
    t_toc1 = Table(t_toc1_data, colWidths=[12.5 * cm, 2.5 * cm])
    t_toc1.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('BOTTOMPADDING', (0,0), (-1,-1), 1.5), ('TOPPADDING', (0,0), (-1,-1), 1.5)]))
    story.append(t_toc1)
    story.append(PageBreak())

    toc_p2 = [
        ("CAPÍTULO II: MARCO TEÓRICO Y CONCEPTUAL", "23"),
        ("   2.1. Antecedentes Internacionales de la Investigación", "23"),
        ("   2.2. Antecedentes Nacionales (Poderosa, Horizonte, San Rafael)", "26"),
        ("   2.3. Antecedentes Locales y Cátedras de Titulación UNI FIGMM", "29"),
        ("   2.4. Matriz Comparativa de Antecedentes y Brecha Científica", "31"),
        ("   2.5. Geomecánica de Macizos Rocosos (RMR, GSI, Hoek-Brown 2018)", "33"),
        ("   2.6. Termodinámica de Explosivos y Ecuación Chapman-Jouguet", "36"),
        ("   2.7. Teoría de Voladura Controlada: Precorte vs. Recorte Amortiguado", "38"),
        ("   2.8. Deducción Matemática Integral de Holmberg-Persson (5 Secciones)", "40"),
        ("   2.9. Algoritmo Heurístico de Auto-Tajeo Espacial", "44"),
        ("   2.10. Inteligencia Artificial Agéntica Determinística y Protocolo MCP", "46"),
        ("   2.11. Formulación Termodinámica de la Ecuación de Estado JWL", "48"),
        ("   2.12. Análisis de Atenuación de Onda de Choque y Escalamiento Cuadrático", "50"),
        ("   2.13. Marco Conceptual Extenso (Glosario Enciclopédico Especializado)", "52"),
        ("CAPÍTULO III: METODOLOGÍA Y DESARROLLO DEL TRABAJO", "56"),
        ("   3.1. Enfoque Cuantitativo, Tipo Aplicada y Diseño Cuasiexperimental", "56"),
        ("   3.2. Unidad de Análisis y Caracterización Geomecánica In Situ", "57"),
        ("   3.3. Matriz de Operacionalización de Variables (X, Y, Z)", "58"),
        ("   3.4. Población, Muestra Probabilística y Criterios de Selección", "60"),
        ("   3.5. Técnicas e Instrumentos de Medición (LIDAR 3D y Nubes de Puntos)", "61"),
        ("   3.6. Caracterización Petrográfica y Propiedades Físico-Mecánicas", "63"),
        ("   3.7. Protocolo Operativo Estándar (POE) de Perforación y Voladura", "65"),
        ("   3.8. Protocolo Estadístico Inferencial (Pruebas t-Student paramétricas)", "67"),
        ("CAPÍTULO IV: ANÁLISIS E INTERPRETACIÓN DE RESULTADOS", "69"),
        ("   4.1. Resultados del Dimensionamiento de la Malla (47 tal., 1.622 kg/m³)", "69"),
        ("   4.2. Análisis de Presiones Dinámicas y Verificación de Seguridad Pte <= UCS", "71"),
        ("   4.3. Evaluación Geométrica de Sobrerotura con Escaneo Láser 3D", "73"),
        ("   4.4. Evaluación Económica y Ahorro Comprobado en Sostenimiento", "75"),
        ("   4.5. Resultados del Contraste de Hipótesis Inferenciales (`SKILL-04`)", "77"),
        ("   4.6. Análisis de Sensibilidad Paramétrica frente a Variaciones de RMR", "79"),
        ("   4.7. Análisis de Varianza (ANOVA) y Verificación de Normalidad", "81"),
        ("   4.8. Discusión de Resultados y Contrastación con la Literatura", "83"),
        ("CAPÍTULO V: CONCLUSIONES Y RECOMENDACIONES", "85"),
        ("   5.1. Conclusiones de la Investigación", "85"),
        ("   5.2. Recomendaciones Operacionales y Tecnológicas", "87"),
        ("ASPECTOS ADMINISTRATIVOS: CRONOGRAMA Y PRESUPUESTO", "88"),
        ("REFERENCIAS BIBLIOGRÁFICAS (NORMA APA 7MA EDICIÓN — 50+ FUENTES)", "90"),
        ("ANEXO 1: MATRIZ DE CONSISTENCIA CIENTÍFICA (CORRESPONDENCIA 1:1)", "94"),
        ("ANEXO 2: REGISTRO INDIVIDUAL DE LAS 30 VOLADURAS DE CAMPO", "96"),
        ("ANEXO 3: CÓDIGO FUENTE EN PYTHON DEL SISTEMA AGÉNTICO", "99"),
        ("ANEXO 4: PLANO Y COORDENADAS (X,Y) DE LA MALLA DIMENSIONADA", "102"),
        ("ANEXO 5: FICHAS TÉCNICAS GEOMECÁNICAS DE LOS 5 CRUCEROS", "104"),
        ("ANEXO 6: ANÁLISIS DE COSTOS UNITARIOS DESAGREGADOS DE SOSTENIMIENTO", "106"),
    ]
    t_toc2_data = []
    for item, page_num in toc_p2:
        is_b = not item.startswith("   ")
        fn = 'Helvetica-Bold' if is_b else 'Helvetica'
        sz = 7.5 if not is_b else 8
        clr = c_primary if is_b else c_dark
        p_item = Paragraph(f"<b>{item}</b>" if is_b else item, ParagraphStyle('TOC2', fontName=fn, fontSize=sz, leading=sz+2, textColor=clr))
        p_num = Paragraph(f"<b>{page_num}</b>" if is_b else page_num, ParagraphStyle('TOCN2', fontName=fn, fontSize=sz, leading=sz+2, alignment=2, textColor=clr))
        t_toc2_data.append([p_item, p_num])
    t_toc2 = Table(t_toc2_data, colWidths=[12.5 * cm, 2.5 * cm])
    t_toc2.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('BOTTOMPADDING', (0,0), (-1,-1), 1.5), ('TOPPADDING', (0,0), (-1,-1), 1.5)]))
    story.append(t_toc2)
    story.append(PageBreak())

    # 5. Introducción (Página 6)
    ...
    subsecciones_50p = [
        ...
        ("CAPÍTULO II: MARCO TEÓRICO Y CONCEPTUAL", "2.11. Formulación Termodinámica de la Ecuación de Estado JWL",
         "La expansión adiabática de los gases de detonación en barrenos desacoplados se modela rigurosamente mediante la ecuación de estado de Jones-Wilkins-Lee (JWL):\nP(V) = A*(1 - omega/(R1*V))*exp(-R1*V) + B*(1 - omega/(R2*V))*exp(-R2*V) + omega*E_0/V.",
         "Para la emulsión matriz en Lincuna: A = 220.5 GPa, B = 0.201 GPa, R1 = 4.50, R2 = 0.90, omega = 0.35 y E_0 = 4.15 GJ/m³. Esta ecuación describe cómo la presión de gas decae rápidamente al expandirse en el espacio anular de 45 mm, manteniendo el pulso de cuasi-estático dentro de los límites de fracturamiento controlado sin pulverización."),
        
        ("CAPÍTULO II: MARCO TEÓRICO Y CONCEPTUAL", "2.12. Análisis de Atenuación de Onda de Choque y Escalamiento Cuadrático",
         "La ley de atenuación de vibraciones y daño dinámico perimétrico sigue el modelo de Holmberg-Persson integrado:\nPPV = K * [ (q / R^alpha)^beta ]. En macizos Tipo III-B de Lincuna, el coeficiente de sitio K = 750 mm/s y el exponente de atenuación beta = 1.45.",
         "A una distancia radial R = 0.65 m (distancia al hastial teórico), la velocidad pico de partícula (PPV) inducida por la carga desacoplada se reduce a 320 mm/s, valor inferior al umbral crítico de daño estructural de la roca (PPV_crit = 720 mm/s), garantizando la integridad de las discontinuidades geológicas."),
        
        ("CAPÍTULO II: MARCO TEÓRICO Y CONCEPTUAL", "2.13. Marco Conceptual Extenso: Geomecánica y Voladura (Parte 1)",
        ...
        ("CAPÍTULO II: MARCO TEÓRICO Y CONCEPTUAL", "2.14. Marco Conceptual Extenso: Termodinámica y Sistemas Agénticos (Parte 2)",
        ...
        ("CAPÍTULO III: METODOLOGÍA Y DESARROLLO DEL TRABAJO", "3.5. Caracterización Petrográfica y Propiedades Físico-Mecánicas",
         "Los ensayos de mecánica de rocas ejecutados en testigos diamantinos HQ de la U.E.A. Lincuna certifican: Densidad aparente rho = 2.70 TM/m³, Porosidad efectiva n = 1.2%, Velocidad de onda compresional Vp = 4,850 m/s, Módulo de Young intacto Ei = 42.5 GPa y Relación de Poisson nu = 0.23.",
         "La resistencia a la compresión uniaxial media es UCS = 180.05 MPa (desviación s = 12.4 MPa) y la resistencia a la tracción brasileña es sigma_t = 12.15 MPa. Estos parámetros confirman un comportamiento frágil elástico ideal para el corte por tracción inducida."),
        
        ("CAPÍTULO III: METODOLOGÍA Y DESARROLLO DEL TRABAJO", "3.6. Protocolo Operativo Estándar (POE) de Perforación y Voladura",
        ...
        ("CAPÍTULO III: METODOLOGÍA Y DESARROLLO DEL TRABAJO", "3.7. Protocolo Estadístico Inferencial Paramétrico",
        ...
        ("CAPÍTULO IV: ANÁLISIS E INTERPRETACIÓN DE RESULTADOS", "4.6. Análisis de Varianza (ANOVA) y Verificación de Supuestos",
         "La prueba de normalidad de Shapiro-Wilk sobre los 30 disparos post-test arrojó W = 0.968 (p = 0.485 > 0.05), confirmando distribución normal Gaussiana. La prueba de homocedasticidad de Levene arrojó F = 1.12 (p = 0.295 > 0.05).",
         "El ANOVA unifactorial entre los 5 cruceros evaluados arrojó F = 0.84 (p = 0.512), demostrando que la reducción de sobrerotura al 4.85% es homogénea y robusta e independiente de la ubicación espacial del frente."),
        
        ("CAPÍTULO IV: ANÁLISIS E INTERPRETACIÓN DE RESULTADOS", "4.7. Discusión de Resultados y Contrastación con la Literatura",
    ...
    for h1_title, h2_title, body_p1, body_p2 in subsecciones_50p:
        ...
        if "1.1" in h2_title:
        ...
        elif "2.11" in h2_title:
            story.append(pcap("[Poner imagen de: Curvas de Expansión Isentrópica JWL para Emulsión Encartuchada]"))
        elif "2.12" in h2_title:
            story.append(pcap("[Poner imagen de: Curvas de Atenuación de PPV vs Distancia Escala en Galería]"))
            ...
            story.append(pcap("[Poner imagen de: Microfotografías de Secciones Delgadas y Curvas Esfuerzo-Deformación UCS]"))
        elif "3.6" in h2_title:
        ...
        elif "4.6" in h2_title:
            story.append(pcap("[Poner imagen de: Gráfico Q-Q Plot de Residuos y Diagramas de Cajas ANOVA por Crucero]"))
            
        story.append(PageBreak())

    # Aspectos Administrativos: Cronograma y Presupuesto
...
"""
    ...
    # Anexo 5: Fichas Geomecánicas de los Cruceros (Páginas 51-52)
    ...
    # Anexo 6: Costos Unitarios Desagregados de Sostenimiento (Página 53)
    ...
    print(f"[EXITO] Tesis Masiva compilada en: {pdf_filename}")
