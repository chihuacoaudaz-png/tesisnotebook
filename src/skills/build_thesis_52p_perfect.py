# -*- coding: utf-8 -*-
"""
COMPILADOR INTEGRAL Y AUTÓNOMO DE TESIS OFICIAL UNI FIGMM (52-54 PÁGINAS EXACTAS)
Diseñado con prosa técnica continua, sin saltos artificiales entre subtítulos.
Incluye demostraciones matemáticas completas, geomecánica de Lincuna,
balance energético, estadística inferencial y APU auditado de shotcrete.
"""
...
def build_thesis_with_target_pages(pdf_filename="output/TESIS_OFICIAL_UNI_LINCUNA_50PAGS.pdf", target_pages=52):
    ...
    style_h1 = ParagraphStyle('Heading1_Perfect', fontName='Helvetica-Bold', fontSize=11.5, leading=15, textColor=c_primary, spaceBefore=14, spaceAfter=6, keepWithNext=True)
    style_h2 = ParagraphStyle('Heading2_Perfect', fontName='Helvetica-Bold', fontSize=10, leading=13, textColor=c_secondary, spaceBefore=10, spaceAfter=4.5, keepWithNext=True)
    style_h3 = ParagraphStyle('Heading3_Perfect', fontName='Helvetica-Bold', fontSize=8.8, leading=12, textColor=c_dark, spaceBefore=8, spaceAfter=3.5, keepWithNext=True)
    style_body = ParagraphStyle('Body_Perfect', fontName='Helvetica', fontSize=8.5, leading=12.2, alignment=4, textColor=c_dark, spaceAfter=5.5)
    style_bullet = ParagraphStyle('Bullet_Perfect', fontName='Helvetica', fontSize=8.5, leading=12, alignment=4, leftIndent=12, textColor=c_dark, spaceAfter=3.5)
    style_eq = ParagraphStyle('Equation_Perfect', fontName='Helvetica-Bold', fontSize=8.0, leading=11, alignment=1, textColor=c_primary, spaceBefore=4.5, spaceAfter=5.5)
    style_caption = ParagraphStyle('Caption_Perfect', fontName='Helvetica-Oblique', fontSize=7.2, leading=9.5, alignment=1, textColor=c_muted, spaceBefore=3.0, spaceAfter=6.0)
    style_th = ParagraphStyle('TableHeader_Perfect', fontName='Helvetica-Bold', fontSize=7.0, leading=9.0, alignment=1, textColor=colors.white)
    style_td = ParagraphStyle('TableCell_Perfect', fontName='Helvetica', fontSize=6.8, leading=8.5, textColor=c_dark)
    style_code = ParagraphStyle('Code_Perfect', fontName='Courier', fontSize=6.5, leading=8.2, textColor=c_primary)
    ...
    # Cargar contenido de los capítulos completamente expandidos
    from generate_fully_expanded_52p_content import append_fully_expanded_content as append_sections
...
if __name__ == "__main__":
    build_thesis_with_target_pages()
